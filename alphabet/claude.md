...
### Animations
- `popIn` — letter entrance (scale + slight rotation bounce)
- `slideIn` / `imgBounce` — word panel and image entrance
- `rippleOut` — expanding ring on each keypress
- `floatUp` — colored particles burst
- All animated elements self-clean via `animationend` listeners.

### Helper patterns
- `restartAnimation(el, className)` — removes class, forces reflow, re-adds class to retrigger CSS animations.
...
|---|---|
| Change a word association | `alphabetWords` in `app.js` |
...
| Modify animations | Edit the `@keyframes` blocks in `style.css` |
